...
class ExperienceController extends Controller {
 var $_controller='experience';	
 var $_title="Kinh nghiệm làm việc";
...
public function loadData(Request $request){
  $query=DB::table('experience');  
  if(!empty(@$request->filter_search)){          
    $query->where('experience.fullname','like','%'.trim(mb_strtolower(@$request->filter_search,'UTF-8')).'%')  ;                   
  }                  
...
public function getForm($task,$id=""){     
  ...
  if(in_array($requestControllerAction, $arrPrivilege)){
    switch ($task) {
     ...
     $arrRowData=ExperienceModel::find((int)@$id)->toArray();                     
     break;
     case 'add':
...
public function save(Request $request){
 ...
 if ($checked == 1) {    
  if(empty($id)){
    ...
    $item       = ExperienceModel::find((int)@$id);   
...
public function updateStatus(Request $request){
  ...
  if($checked==1){
    foreach ($arrID as $key => $value) {
      if(!empty($value)){
        $item=ExperienceModel::find($value);
        $item->status=$status;
        $item->save();      
      }            
    }
...
public function trash(Request $request){
  ...
  if($checked == 1){                                  
    ...
    DB::table('experience')->whereIn('id',@$arrID)->delete();   
    $msg['success']='Xóa thành công';                                     
  }
  $data                   =   $this->loadData($request);
  $info = array(
...
public function sortOrder(Request $request){
  ...
  if(count($data_order) > 0){              
    foreach($data_order as $key => $value){      
      if(!empty($value)){
        $item=ExperienceModel::find((int)@$value->id);                
...
public function createAlias(Request $request){
  ...
  if (empty($id)) {              
    $dataScale=ExperienceModel::whereRaw("trim(lower(alias)) = ?",[trim(mb_strtolower($alias,'UTF-8'))])->get()->toArray();             
  }else{
    $dataScale=ExperienceModel::whereRaw("trim(lower(alias)) = ? and id != ?",[trim(mb_strtolower($alias,'UTF-8')),(int)@$id])->get()->toArray();    
  }  
