...
public function loadData(Request $request){
  $query=DB::table('literacy');  
  if(!empty(@$request->filter_search)){          
    $query->where('literacy.fullname','like','%'.trim(mb_strtolower(@$request->filter_search,'UTF-8')).'%')  ;                   
  }                  
...
public function getForm($task,$id=""){     
  ...
  if(in_array($requestControllerAction, $arrPrivilege)){
    switch ($task) {
     ...
     $arrRowData=LiteracyModel::find((int)@$id)->toArray();                     
     break;
     case 'add':
...
public function save(Request $request){
 ...
 if ($checked == 1) {    
  if(empty($id)){
    $item         =   new LiteracyModel;       
    $item->created_at   = date("Y-m-d H:i:s",time());        

  } else{
    $item       = LiteracyModel::find((int)@$id);   
...
public function trash(Request $request){
  ...
  if($checked == 1){                                  
    ...
    DB::table('literacy')->whereIn('id',@$arrID)->delete();   
    $msg['success']='Xóa thành công';                                     
  }
  $data                   =   $this->loadData($request);
  $info = array(
...
public function sortOrder(Request $request){
  ...
  if(count($data_order) > 0){              
    foreach($data_order as $key => $value){      
      if(!empty($value)){
        $item=LiteracyModel::find((int)@$value->id);                
...
public function createAlias(Request $request){
  ...
  if (empty($id)) {              
    $dataScale=LiteracyModel::whereRaw("trim(lower(alias)) = ?",[trim(mb_strtolower($alias,'UTF-8'))])->get()->toArray();             
  }else{
    $dataScale=LiteracyModel::whereRaw("trim(lower(alias)) = ? and id != ?",[trim(mb_strtolower($alias,'UTF-8')),(int)@$id])->get()->toArray();    
  }  
