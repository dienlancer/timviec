...
$ddlProject     	=   cmsSelectboxCategory('project_id', 'form-control', $arrProject, 0,"");
...
<form class="form-horizontal" role="form" name="frm">	
	...
	<div class="portlet light bordered">
		...
		<div class="portlet-body">		
			<table class="table table-striped table-bordered table-hover table-checkable order-column" id="tbl-project-article">
				<thead>
					<tr>
...
<script type="text/javascript" language="javascript">	
	...
	function getList() {  	
		...
		$.ajax({
			...
			success: function (data, status, jqXHR) {  				
				vProjectArticleTable.clear().draw();
				vProjectArticleTable.rows.add(data).draw();
				spinner.hide();
			},
	...
	function checkWithList(this_checkbox){
		var dr = vProjectArticleTable.row( $(this_checkbox).closest('tr') ).data();       		
		...
		vProjectArticleTable.row( $(this_checkbox).closest('tr') ).data(dr);
	}	
	function changeStatus(id,status){		
		...
		$.ajax({
			...
			success: function (data, status, jqXHR) {   							                              				
				...
				vProjectArticleTable.clear().draw();
				vProjectArticleTable.rows.add(data.data).draw();
				spinner.hide();
			},
			beforeSend  : function(jqXHR,setting){
				spinner.show();
	...
	function updateStatus(status){				
		var token 	= 	$('input[name="_token"]').val();   
		var project_id=$('select[name="project_id"]').val();
		var dt 		= 	vProjectArticleTable.data();		
		console.log(dt);
		var str_id	=	"";		
